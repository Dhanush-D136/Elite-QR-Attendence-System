...
async function createStudent(req, res) {
  ...
  db.run(
    `INSERT INTO users (id, name, roll_number, email, role, department, year, section, phone, profile_photo, password_hash, must_change_password)
     VALUES (?, ?, ?, ?, 'student', ?, ?, ?, ?, ?, ?, 1)`,
    ...
    function (err) {
      ...
      res.status(201).json({
        message: 'Student account created successfully with default password "1234"',
...
async function updateStudent(req, res) {
  ...
  try {
    if (new_password && new_password.trim() !== '') {
      ...
      db.run(
        `UPDATE users 
         SET name = ?, roll_number = ?, email = ?, department = ?, year = ?, section = ?, phone = ?, profile_photo = COALESCE(?, profile_photo), password_hash = ?, must_change_password = 0
